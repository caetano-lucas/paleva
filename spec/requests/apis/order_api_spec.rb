...
describe 'Orders API' do
  ...
  context 'PATCH api/v1/restaurant/1/orders/1' do
    it 'success' do 
      cpf = CPF.generate(true).split
      cpf2 = CPF.generate(true).split
      cnpj = CNPJ.generate(true).split
      ...
      patch "/api/v1/restaurants/#{restaurant.alphanumeric_code}/orders/#{order.alphanumeric_code}?status=3"

      expect(response.status).to eq 200
      expect(response.content_type).to include 'application/json'

      json_response = JSON.parse(response.body)
      expect(json_response['order']["status"]).to eq('pronto')
    end
